...
def get_word_score(word, n):
    ...
    return component1*abs(component2)
...
def display_hand(hand):
    ...
    hand_a=hand.copy()
    keys=''
    for letter in hand_a.keys():
        for j in range(hand[letter]):
              keys=keys+letter+' '   # print all on the same line
    return keys                            # print an empty line
...
def update_hand(hand, word):
    ...
    words_uniq=''
    for letter in word:
        if letter not in words_uniq:
            if letter in hand_c:
               hand_c[letter]=hand_c[letter]-word.count(letter)
            if hand_c[letter]<0:
                hand_c[letter]=0
            if hand_c[letter]==0:
                del hand_c[letter]
        words_uniq=words_uniq+letter
...
def is_valid_word(word, hand, word_list):
    ...
    for letter in word:
        if letter not in keys:
            # print('yes0')
            return False
        elif letter in keys and word.count(letter) > keys.count(letter):
            # print('yes1')
            return False
    if word in word_list:
        ...
        # print('yes3')
        word=word.strip('*')
        for word_inlist in word_list:
            if len(word)==len(word_inlist):
                for letter in word:
                    if letter not in word_inlist:
                        return False
                    else:
                        return True
...
def substitute_hand(hand, letter):
    ...
    for letter_u in hand:
        if letter_u in vowels:
           vowels=vowels.replace(letter_u,'')
        else:
           consonents=consonents.replace(letter_u,'')
    if  letter in VOWELS:
        rletter= random.choice(vowels)
    elif letter in CONSONANTS:
        rletter=random.choice(consonents)
        
    if letter not in hand:
        pass
    else:
        keys=''
        for key in hand_sc.keys():
            for value in range(hand_sc[key]):
                keys=keys+key
                
        keys_s=''.join(keys)
        keys_s=keys_s.replace(letter,rletter)
        
        
    return get_frequency_dict(keys_s)
